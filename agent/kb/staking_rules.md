...
1. **Flexible Staking:**
   * **Mechanism:** Assets remain available for trading or withdrawal at any time. There is no lock-up period.
   * **Reward Calculation:** Daily snapshots of the user's spot wallet are taken at 00:00 UTC. Rewards accumulate daily and are distributed on the 1st of every calendar month.
   * **Yield:** Lower Annual Percentage Yield (APY) compared to Locked options.

2. **Locked Staking:**
   * **Mechanism:** Assets are securely committed to a smart contract or validation node for a fixed duration (15, 30, 60, or 90 days).
   * **Reward Calculation:** Interest accrues daily starting from 00:00 UTC the day after the subscription is finalized. Distributed daily directly to the user's Staking Wallet.
   * **Early Redemption:** Users may request early redemption of locked assets. However, if early redemption is triggered, **all accumulated rewards for that specific staking term will be forfeited**. The principal amount will be returned to the Spot Wallet after a standard network unbonding period of 48 to 72 hours.

### Operational Restrictions and Eligibility
